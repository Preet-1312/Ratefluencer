        ...
        for story_id in story_ids:
            try:
                ...
                if story_res.status_code == 200:
                    ...
                    # Filter keywords to make it highly relevant
                    keywords = ["ai", "tech", "startup", "developer", "programming", "llm", "gpt", "model", "code", "software", "web", "app", "cloud", "saas", "hacker"]
        ...
        if response.status_code == 200:
            ...
            for item in items[:10]:
                ...
                trends.append({
                    "source": "TechCrunch",
                    "title": title_text,
                    "score": 150, # Static high score for TechCrunch front page stories
                    "comments": 10,
                    "url": link_text
                })
    except Exception as e:
...
if __name__ == "__main__":
    ...
    hn = get_hacker_news_trends()
    print(f"Fetched {len(hn)} HackerNews items:")
    print(json.dumps(hn, indent=2))
    
    print("\nFetching Dev.to...")
    dev = get_dev_to_trends()
    print(f"Fetched {len(dev)} Dev.to items:")
    print(json.dumps(dev, indent=2))
    
    print("\nFetching TechCrunch...")
    tc = get_techcrunch_trends()
    print(f"Fetched {len(tc)} TechCrunch items:")
